...
// UnmarshalJSON implements json.Unmarshaler.
func (j *TransferBetweenAccountsRespAccountsElemProduct) UnmarshalJSON(b []byte) error {
	var v string
	if err := json.Unmarshal(b, &v); err != nil {
		return err
	}
	var ok bool
	for _, expected := range enumValues_TransferBetweenAccountsRespAccountsElemProduct {
		if reflect.DeepEqual(v, expected) {
			ok = true
			break
		}
	...
	if !ok {
		return fmt.Errorf("invalid value (expected one of %#v): %#v", enumValues_TransferBetweenAccountsRespAccountsElemProduct, v)
	}
	*j = TransferBetweenAccountsRespAccountsElemProduct(v)
	return nil
}
